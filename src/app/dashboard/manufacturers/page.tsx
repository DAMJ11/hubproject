  ...
  const loadManufacturers = useCallback(async () => {
    ...
    try {
      ...
      if (!manufacturersData.success) {
        setError(manufacturersData.message || "No se pudieron cargar los fabricantes");
        setManufacturers([]);
        return;
      }
      ...
      setError("Error de red cargando fabricantes");
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Directorio de fabricantes</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400">
          Explora empresas, filtra por criterios clave y abre contacto por chat con un click.
        </p>
      </div>

      {role && role !== "brand" && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
          Esta vista fue pensada para marcas. Puedes navegar el directorio, pero el flujo principal de contacto esta optimizado para brand.
        </div>
      )}
        ...
        <div className="mb-3 flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-200">
          ...
          {isRefreshing && (
            <span className="inline-flex items-center gap-1 text-xs font-normal text-[#2563eb]">
              <Loader2 className="h-3.5 w-3.5 animate-spin" /> Actualizando resultados...
            </span>
          )}
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-gray-200 bg-white py-16 text-center dark:border-slate-700 dark:bg-slate-800">
          ...
          <p className="text-sm text-gray-500">No encontramos fabricantes con esos filtros.</p>
        </div>
      ) : (
        <div className="grid gap-4 lg:grid-cols-2">
          {manufacturers.map((manufacturer) => {
            const chatParams = new URLSearchParams({
              ...
              subject: `Explorar colaboracion con ${manufacturer.name}`,
              message: `Hola ${manufacturer.name}, nos interesa explorar una colaboracion para nuestros proyectos activos.`,
            });
            ...
            return (
              ...
              >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {[manufacturer.city, manufacturer.state, manufacturer.country].filter(Boolean).join(" · ") || "Ubicacion no disponible"}
                      </p>
                    </div>
                  </div>
                  <span className="rounded-full bg-blue-50 px-2 py-1 text-xs font-medium text-blue-700">
                    {manufacturer.awardedProjectsCount} adjudicados
                  </span>
                </div>

                <p className="mb-3 line-clamp-3 text-sm text-gray-600 dark:text-gray-300">
                  {manufacturer.description || "Sin descripcion publica."}
                </p>

                <div className="mb-3 flex flex-wrap gap-2">
                  <span className="rounded-full bg-emerald-50 px-2 py-1 text-xs text-emerald-700">
                    {manufacturer.certificationsCount} certificaciones
                  </span>
                  <span className="rounded-full bg-amber-50 px-2 py-1 text-xs text-amber-700">
                    {manufacturer.verifiedCertificationsCount} certificaciones verificadas
                  </span>
                  {manufacturer.foundedYear && (
                    <span className="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-700">
                      Fundada en {manufacturer.foundedYear}
                    </span>
                  )}
                  {manufacturer.employeeCount && (
                    <span className="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-700">
                      Equipo {manufacturer.employeeCount}
                    </span>
                  )}
                </div>
                ...
                <div className="mb-4">
                  ...
                  >
                    {expandedOfferCompanyId === manufacturer.id ? "Ocultar oferta tecnica" : "Ver oferta tecnica y comercial"}
                  </Button>
                    ...
                    <div className="mt-2 rounded-lg border border-gray-200 p-3 dark:border-slate-600">
                      {offersLoadingCompanyId === manufacturer.id ? (
                        <div className="text-xs text-gray-500 flex items-center gap-2">
                          <Loader2 className="h-3.5 w-3.5 animate-spin" /> Cargando oferta...
                        </div>
                      ) : (offersByCompany[manufacturer.id]?.length ?? 0) === 0 ? (
                        <p className="text-xs text-gray-500">Este fabricante aun no publico capacidades detalladas.</p>
                      ) : (
                          ...
                          {(offersByCompany[manufacturer.id] ?? []).map((offer) => (
                            <div key={offer.id} className="rounded-md bg-gray-50 p-2 dark:bg-slate-700/40">
                              ...
                              <p className="text-xs text-gray-600 dark:text-gray-300 mt-0.5">
                                MOQ {offer.min_order_qty}
                                {offer.max_monthly_capacity ? ` · Capacidad ${offer.max_monthly_capacity.toLocaleString()}/mes` : ""}
                                {offer.lead_time_days ? ` · Entrega ${offer.lead_time_days} dias` : ""}
                              </p>
                              {(offer.unit_price_from !== null || offer.wholesale_price_from !== null) && (
                                <p className="text-xs text-blue-700 mt-1">
                                  {offer.unit_price_from !== null ? `Desde ${formatCOP(offer.unit_price_from)} por prenda` : ""}
                                  {offer.unit_price_from !== null && offer.wholesale_price_from !== null ? " · " : ""}
                                  {offer.wholesale_price_from !== null ? `Mayorista desde ${formatCOP(offer.wholesale_price_from)}` : ""}
                                </p>
                              )}
                ...
                {canContactManufacturers ? (
                  <Link href={`/dashboard/messages?${chatParams.toString()}`}>
                    <Button className="w-full bg-[#2563eb] text-white hover:bg-[#1d4ed8]">
                      <MessageSquare className="mr-2 h-4 w-4" /> Contactar por chat
                    </Button>
                  </Link>
                ) : (
                  <Button className="w-full" variant="outline" disabled>
                    Contacto disponible para cuentas brand
                  </Button>
                )}
              </div>
            );
      ...
      <div className="rounded-lg border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-800">
        <div className="flex items-start gap-2">
          ...
          <p>
            Consejo: al iniciar el chat, en Mensajes podras elegir el proyecto activo que quieres ofrecer al fabricante para mantener el contexto del contacto.
          </p>
        </div>
      </div>
    </div>
