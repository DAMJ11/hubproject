  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{isAdmin ? "Rese&ntilde;as" : "Mis Rese&ntilde;as"}</h1>
          <p className="text-gray-500 mt-1">{isAdmin ? "Todas las rese&ntilde;as de la plataforma" : "Rese&ntilde;as que has dejado"}</p>
        </div>
        <Card className="px-4 py-2 flex items-center gap-2">
          <Star className="w-5 h-5 text-yellow-400 fill-yellow-400" />
          <span className="text-xl font-bold">{avgRating}</span>
          <span className="text-sm text-gray-500">/ 5.0 ({reviews.length} rese&ntilde;as)</span>
        </Card>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Buscar rese&ntilde;as..." className="pl-10" />
        </div>
        <div className="flex gap-2">
          <Button variant={filterRating === 0 ? "default" : "outline"} size="sm" onClick={() => setFilterRating(0)}
            className={filterRating === 0 ? "bg-[#2563eb] hover:bg-[#1d4ed8]" : ""}>Todas</Button>
          {[5, 4, 3, 2, 1].map((r) => (
            <Button key={r} variant={filterRating === r ? "default" : "outline"} size="sm" onClick={() => setFilterRating(r)}
              className={filterRating === r ? "bg-[#2563eb] hover:bg-[#1d4ed8]" : ""}>
        ...
        {filtered.length === 0 && (
          <Card className="p-8 text-center">
            <p className="text-sm font-medium text-gray-700">
              {reviews.length === 0 ? "Aun no hay resenas registradas." : "No se encontraron resenas con esos filtros."}
            </p>
            <p className="text-xs text-gray-500 mt-1">
              {reviews.length === 0 ? "Cuando tus clientes dejen comentarios apareceran aqui." : "Ajusta la busqueda o el filtro de estrellas."}
            </p>
        ...
        {filtered.map((review) => (
          <Card key={review.id} className="p-5">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">{new Date(review.created_at).toLocaleDateString("es-CO", { year: "numeric", month: "long", day: "numeric" })}</p>
                </div>
              </div>
            ...
            <div className="mb-3">
              ...
              <span className="text-xs text-gray-500">Especialista: {review.professional_name}</span>
            </div>

            <p className="text-gray-700 text-sm leading-relaxed">{review.comment}</p>

            <div className="flex items-center gap-4 mt-4 pt-3 border-t">
              <button className="flex items-center gap-1 text-xs text-gray-500 hover:text-[#2563eb]">
                <ThumbsUp className="w-3.5 h-3.5" /> &Uacute;til
              </button>
              <button className="flex items-center gap-1 text-xs text-gray-500 hover:text-[#2563eb]">
