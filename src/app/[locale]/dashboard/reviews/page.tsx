      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2">
          <Button variant={filterRating === 0 ? "default" : "outline"} size="sm" onClick={() => setFilterRating(0)}
            className={filterRating === 0 ? "bg-[#2563eb] hover:bg-[#1d4ed8]" : ""}>{t("all")}</Button>
          {[5, 4, 3, 2, 1].map((r) => (
        ...
        {filtered.length === 0 && (
          <Card className="p-8 text-center">
            <p className="text-sm font-medium text-gray-700">
              {reviews.length === 0 ? t("emptyNoReviews") : t("emptyFiltered")}
            </p>
            <p className="text-xs text-gray-500 mt-1">
              {reviews.length === 0 ? t("emptyNoReviewsHint") : t("emptyFilteredHint")}
            </p>
        ...
        {filtered.map((review) => (
          <Card key={review.id} className="p-5">
            ...
            <div className="flex items-center gap-4 mt-4 pt-3 border-t">
              ...
              {!review.is_public && (
                <span className="text-xs text-[#2563eb] ml-auto">{t("private")}</span>
              )}
            </div>
          </Card>
        ))}
