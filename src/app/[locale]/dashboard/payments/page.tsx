...
const statusConfig: Record<string, { color: string; icon: typeof CheckCircle }> = {
  ...
  refunded: { color: "bg-blue-100 text-blue-700", icon: ArrowDownRight },
};
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b">
                <th className="text-left p-4 text-sm font-medium text-gray-600">{t("table.code")}</th>
                <th className="text-left p-4 text-sm font-medium text-gray-600">{t("table.client")}</th>
                <th className="text-left p-4 text-sm font-medium text-gray-600">{t("table.service")}</th>
                <th className="text-left p-4 text-sm font-medium text-gray-600">{t("table.amount")}</th>
                <th className="text-left p-4 text-sm font-medium text-gray-600">{t("table.method")}</th>
                <th className="text-left p-4 text-sm font-medium text-gray-600">{t("table.status")}</th>
                <th className="text-left p-4 text-sm font-medium text-gray-600">{t("table.date")}</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={7} className="p-8 text-center">
                    <p className="text-sm font-medium text-gray-700">
                      {payments.length === 0 ? t("emptyNoPayments") : t("emptyNoResults")}
                    </p>
                    <p className="text-xs text-gray-500 mt-1">
                      {payments.length === 0 ? t("emptyHint") : t("filterHint")}
                    </p>
              ...
              {filtered.map((p) => {
                ...
                return (
                  <tr key={p.id} className="border-b hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-4 text-sm text-gray-500">{p.paid_at ? new Date(p.paid_at).toLocaleDateString(locale) : new Date(p.created_at).toLocaleDateString(locale)}</td>
                  </tr>
                );
              })}
            </tbody>
